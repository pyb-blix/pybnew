...
class UserController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['all_message'] = $all_message = Messages::whereIn('id', function($query) use ($user_id) {
            ...
            ->with(['reservation' => function($query) {
                $query->with(['currency', 'rooms' => function($query) {
        ...
        $data['unread'] = $all_message->where('archive','0')->reject(function ($unread) {
        ...
        //future payouts
        $host_future_payout = payouts::join('reservation', function($join) {
        ...
        $future_payout = 0;
        $future_nights = 0;
        $total_rooms = $host_future_payout->count();

        foreach($host_future_payout as $future_payouts) {   
            $future_payout += $future_payouts->amount;
            $future_nights += $future_payouts->nights;      
        }

        $host_completed_payout = payouts::join('reservation', function($join) {
                $join->on('reservation.id', '=', 'payouts.reservation_id');
            })
        ...
        $completed_payout=0;
        $completed_nights=0;
        $total_rooms1 = $host_completed_payout->count();

        foreach($host_completed_payout as $completed_payouts) {   
            $completed_payout += $completed_payouts->amount;
            $completed_nights += $completed_payouts->nights;
        }
        
        //current year earnings
        $host_year_payout = payouts::select('payouts.amount','payouts.currency_code')
        ...
        foreach($host_year_payout as $total_payouts) {   
        ...
        $data['total_rooms'] = $total_rooms+$total_rooms1;
